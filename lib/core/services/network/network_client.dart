...
class NetworkClient {
  ...
  final VoidCallback onUnAthorized;
  final Map<String, String> Function() commonHeaders;
  final String defaultErrorMessage = 'Something went Wrong';

  NetworkClient({required this.onUnAthorized, required this.commonHeaders});
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        return NetworkResponse(
          ...
          statusCode: response.statusCode,
          errorMessage: 'UnAthorized',
          responseData: responseBody['msg'] ?? defaultErrorMessage,
        );
      }
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        return NetworkResponse(
          ...
          statusCode: response.statusCode,
          errorMessage: 'UnAthorized',
          responseData: reponseBody['msg'] ?? defaultErrorMessage,
        );
      }
    } on Exception catch (e) {
      return NetworkResponse(
  ...
  Future<NetworkResponse> patchRequest(
    ...
    try {
      ...
      final Response response = await patch(uri, headers: commonHeaders());
      _logResponse(response);
      if (response.statusCode == 200 || response.statusCode == 201) {
        final responseBody = jsonDecode(response.body);
        return NetworkResponse(
